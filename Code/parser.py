...
class Parser:
    ...
    def parse_default_settings(self):
            with open(self.default_settings_path, 'r') as settings:
                data = json.load(settings)
            return data
    ...
    def unparse_default_settings(self):
        ...
        if os.path.getsize(self.default_settings_path) <= 0:
            data = {
    ...
    def parse_credits(self, path):
        ...
        credits_text = []
        font_size = int((1.67 * (self.parse_settings()['width'] + self.parse_settings()['height'])) / 100)
